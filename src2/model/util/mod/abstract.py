...
from ..io.batch import BatchData , BatchOutput , BoosterInput
from .status import TrainerStatus

class _BaseTrainProcess:
...
class BaseCallBack(_BaseTrainProcess):
    ...
    def _assert_validity(cls):
        ...
        assert BaseCallBack in cls.__mro__ , (cls , cls.__mro__)
        self_hooks , base_hooks = cls._possible_hooks() , BaseCallBack._possible_hooks()
...
class BaseTrainer(ABC , _BaseTrainProcess):
    ...
    def __init__(self , **kwargs):
        ...
        if hasattr(self , 'callbacks'):
            [setattr(self , x , self.callbacks(getattr(self , x))) for x in dir(self) if BaseCallBack._possible_hook(x)]
