...
class BoosterInput:
    ...
    def __post_init__(self):
        if self.y is not None: self.finite = np.isfinite(self.y)
        self.update_feature()
    ...
    def concat(cls , datas : list[Optional['BoosterInput']]):
        ...
        new_blk = cls(x , y , secid , date , feature)
        return new_blk
        # return cls.from_dataframe(pd.concat([data.to_dataframe() for data in blocks]))
