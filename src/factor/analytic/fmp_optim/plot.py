...
def plot_optim_frontface(data : pd.DataFrame , show = False):
    ...
    for i , sub_data in enumerate(group_plot):     
        full_title = f'Optim FMP Prefix Information for Factors (P{i+1}/{num_pages})'
    ...
    for i , sub_data in enumerate(group_plot):     
        with PlotFactorData(sub_data , drop = DROP_KEYS , show = show and i == 0, 
            ...
            if 'lag_cost' in df.columns:
                ...
                set_yaxis(ax2 , format='pct' , digits=2 , title='Cummulative Lag Cost' , title_color='r' , tick_color='r' , tick_pos=None)
