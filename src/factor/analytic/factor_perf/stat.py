...
def eval_group_perf(factor_val : DataBlock | pd.DataFrame, benchmark : Optional[Benchmark | str] = None , 
...
def calc_factor_frontface(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None , 
    ...
    ic_stats = eval_ic_stats(ic_df , nday = nday)
    return ic_stats

def calc_factor_ic_curve(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None , 
...
def calc_factor_ic_decay(factor_val : DataBlock | pd.DataFrame, benchmark : Optional[Benchmark | str] = None , 
...
def calc_factor_ic_indus(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None , 
                         nday : int = 10 , lag : int = 2 , ic_type  : Literal['pearson' , 'spearman'] = 'pearson' ,
                         ret_type : Literal['close' , 'vwap'] = 'close'):
    factor_val , secid , date = factor_val_breakdown(factor_val , benchmark)
...
def calc_factor_ic_year(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None , 
                        nday : int = 10 , lag : int = 2 , ic_type  : Literal['pearson' , 'spearman'] = 'pearson' ,
                        ret_type : Literal['close' , 'vwap'] = 'close'):
    factor_val , secid , date = factor_val_breakdown(factor_val , benchmark)
    ic_df = eval_ic(factor_val , nday=nday , lag=lag , ic_type=ic_type , ret_type=ret_type)
    full_rslt = eval_ic_stats(ic_df , nday = nday).assign(year = 'all')

    ic_df['year'] = ic_df.index.astype(str).str[:4]
    year_rslt     = ic_df.groupby(['year']).apply(eval_ic_stats , nday = nday).reset_index(drop=False)

    rtn = pd.concat((year_rslt, full_rslt), axis=0)
    return rtn

def calc_factor_ic_benchmark(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None , 
                          nday : int = 10 , lag : int = 2 , ic_type  : Literal['pearson' , 'spearman'] = 'pearson' ,
                          ret_type : Literal['close' , 'vwap'] = 'close'):
    factor_val , secid , date = factor_val_breakdown(factor_val , benchmark)
    ic_df = eval_ic(factor_val , nday=nday , lag=lag , ic_type=ic_type , ret_type=ret_type)
...
def calc_factor_ic_monotony(factor_val : DataBlock | pd.DataFrame, benchmark : Optional[Benchmark | str] = None , 
                            nday : int = 10 , lag_init : int = 2 , ret_type : Literal['close' , 'vwap'] = 'close'):
    factor_val , secid , date = factor_val_breakdown(factor_val , benchmark)
    grp_perf = eval_group_perf(factor_val , nday = nday , lag = lag_init , group_num = 100 , 
                               excess=True , ret_type = ret_type , trade_date=False)
    n_periods  = 243 / nday
    grp_ret_mean = grp_perf.groupby(['factor_name', 'group'],observed=False)['group_ret'].mean() * n_periods
    grp_ret_std  = grp_perf.groupby(['factor_name', 'group'],observed=False)['group_ret'].std() * np.sqrt(n_periods)
...
def calc_factor_pnl_curve(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None , 
    ...
    direction = np.sign(factor_ret.corr().loc['ret'].drop('ret')) if given_direction is None or given_direction == 0 else given_direction
...
def calc_factor_style_corr(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None):
    factor_val , secid , date = factor_val_breakdown(factor_val , benchmark)
...
def calc_factor_group_curve(factor_val : DataBlock | pd.DataFrame, benchmark : Optional[Benchmark | str] = None , 
                            nday : int = 10 , lag : int = 2 , group_num : int = 10 , excess = False , 
                            ret_type : Literal['close' , 'vwap'] = 'close' , trade_date = True) -> pd.DataFrame:
    df = eval_group_perf(factor_val , benchmark , nday , lag , group_num , excess , ret_type , trade_date).\
        set_index(['factor_name','group'])
    df0 = (df.groupby(df.index.names , observed=False)['date'].min() - 1).to_frame().assign(group_ret = 0.)
    df = pd.concat([df0 , df]).set_index('date' , append=True).sort_values(['group' , 'date'])
    df = df.groupby(['factor_name','group'] , observed=True)['group_ret'].cumsum().rename('cum_ret').reset_index()
    return df

def calc_factor_group_decay(factor_val : DataBlock | pd.DataFrame, benchmark : Optional[Benchmark | str] = None , 
                            nday : int = 10 , lag_init : int = 2 , group_num : int = 10 ,
                            lag_num : int = 5 , ret_type : Literal['close' , 'vwap'] = 'close'):
    decay_grp_perf = []
    factor_val , secid , date = factor_val_breakdown(factor_val , benchmark)
    for lag in range(lag_num):
        grp_perf = eval_group_perf(factor_val , nday = nday , lag = lag_init + lag * nday , 
    ...
    n_periods  = 243 / nday
    decay_grp_ret_mean = decay_grp_perf.groupby(['factor_name', 'group', 'lag_type'],observed=False)['group_ret'].mean() * n_periods
    deacy_grp_ret_std  = decay_grp_perf.groupby(['factor_name', 'group', 'lag_type'],observed=False)['group_ret'].std() * np.sqrt(n_periods)
...
def calc_factor_group_year(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None ,
                           nday : int = 10 , lag : int = 2 , group_num : int = 10 ,
                           ret_type : Literal['close' , 'vwap'] = 'close'):
    factor_val , secid , date = factor_val_breakdown(factor_val , benchmark)
    grp_perf = eval_group_perf(factor_val , nday = nday , lag = lag , 
                               group_num = group_num , ret_type = ret_type , trade_date=False)
...
def calc_factor_distrib_curve(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None , 
...
def calc_factor_distrib_qtile(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark | str] = None , scaling : bool = True):
