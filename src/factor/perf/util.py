...
def eval_grp_avg(x : pd.DataFrame , x_cols : list, y_name : str = 'ret', group_num : int = 10) -> pd.DataFrame:
    y = pd.DataFrame(x[y_name], index=x.index, columns=[y_name])
    rtn = list()
    for col in x_cols:
        y['group'] = pd.qcut(x[col], group_num, labels=[f'group{i}' for i in range(1, group_num + 1)])
...
def calc_grp_perf(factor_val : DataBlock | pd.DataFrame, benchmark : Optional[Benchmark] = None , 
    ...
    factor_val = get_fut_ret(factor_val , nday , lag , ret_type = ret_type)
    factor_list = factor_val.columns.values[:-1]
    df = factor_val.groupby('date').apply(eval_grp_avg , x_cols = factor_list , y_name = 'ret' , group_num = group_num) # type: ignore
...
def calc_top_grp_perf_yearly(factor_val : DataBlock | pd.DataFrame , benchmark : Optional[Benchmark] = None ,
                             nday : int = 10 , lag : int = 2 , group_num : int = 10 ,
                             ret_type : Literal['close' , 'vwap'] = 'close'):
    factor_val , secid , date = factor_val_breakdown(factor_val , benchmark)
