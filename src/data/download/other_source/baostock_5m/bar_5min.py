...
def baostock_secdf(date : int):
    ...
    DB.save_df(secdf , path , vb_level = Proj.vb.max , prefix = 'Baostock Secdf')
    return secdf
        ...
        try:
            ...
            for i , code in enumerate(task_codes):
                ...
                if isinstance(result , pd.DataFrame):
                    DB.save_df(result , tmp_file_path(start_dt , end_dt , code) , vb_level = Proj.vb.max , prefix = f'Baostock 5min codes {i}/{len(task_codes)}')
