"""Tushare fetchers for mutual fund data: fund list, daily NAV, holdings."""
# do not use relative import in this file because it will be running in top-level directory
import numpy as np
import pandas as pd

from src.proj import MACHINE
from src.data.download.tushare.basic import TS , InfoFetcher , TushareFetcher , DayFetcher

class FundInfo(InfoFetcher):
    """mutual fund info"""
    DB_KEY = 'mutual_fund_info'
    UPDATE_FREQ = 'w'

    def get_data(self , date):
        renamer = {'ts_code' : 'fund_id'}
        df1 = self.iterate_fetch(self.api.fund_basic , limit = 5000 , market='E')
        df2 = self.iterate_fetch(self.api.fund_basic , limit = 5000 , market='O')

        df = pd.concat([df1 , df2]).rename(columns=renamer)
        if df.empty: 
            return df
        int_cols = df.columns.intersection(['found_date' , 'due_date' , 'list_date' , 'issue_date' , 'delist_date'])
        df = df.fillna({col:99991231 for col in int_cols}).astype({col:int for col in int_cols}).reset_index(drop=True)
        return df

class FundPortfolioFetcher(TushareFetcher):
    """mutual fund portfolio"""
    START_DATE = 20070101
    DB_TYPE = 'fundport'
    UPDATE_FREQ = 'w'
    DB_SRC = 'holding_ts'
    DB_KEY = 'mutual_fund'
    DATA_FREQ = 'q'
    CONSIDER_FUTURE = False

    def target_dates(self):
        dates = self._fina_fetcher_update_dates(self.DATA_FREQ , self.CONSIDER_FUTURE)
        if len(dates) <= 2: 
            dates = [] # if a lot of dates are missing, ignore update state to update
        return dates
    
    def get_data(self , date):
        renamer = {'ts_code' : 'fund_id'}
        if MACHINE.utc8:
            limit , max_fetch_times = 3000 , 500
        else:
            limit , max_fetch_times = 1000 , 2000
        df = self.iterate_fetch(self.api.fund_portfolio , limit = limit , period = str(date) , max_fetch_times=max_fetch_times , breakpoint = True)
        if df.empty: 
            return df
        df = TS.code_to_secid(df.rename(columns=renamer) , code_col='symbol' , drop_old = False)
        int_cols = df.columns.intersection(['ann_date' , 'end_date'])
        df = df.fillna({col:99991231 for col in int_cols}).astype({col:int for col in int_cols}).reset_index(drop=True)
        return df

class ETFDailyQuote(DayFetcher):
    """ETF daily quote"""
    START_DATE = 20180101 if MACHINE.updatable else 20241215
    DB_KEY = 'etf_day'
    def get_data(self , date : int):
        date_str = str(date)
        
        quote = self.iterate_fetch(self.api.fund_daily , limit = 2000 , trade_date=date_str)
        if quote.empty: 
            return quote
        quote = quote.rename(columns={'pct_change':'pctchange','pre_close':'preclose','vol':'volume'})
        quote['volume'] = quote['volume'] * 1000
        quote['amount'] = quote['amount'] * 10000
        quote['vwap'] = np.where(quote['volume'] == 0 , quote['close'] , quote['amount'] / quote['volume'])
        return quote