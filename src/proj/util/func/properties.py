...
def empty(*objs : Any) -> bool:
    """True if every object is considered empty (None, ``.empty``, zero size/len, falsy)."""
    obj_empty : list[bool] = []
    for obj in objs:
        if obj is None:
...
def shape(obj : Any , keys : list[str] | None = None) -> Any:
    """Recursive ``.shape`` / collection structure; ``keys`` selects sub-fields."""
    if obj is None: 
        return []
...
def max_of_date(date : Any) -> int: 
    """Largest YYYYMMDD int in ``date``; ``-1`` if empty."""
    if date is None or len(date) == 0:
        return -1 
    else:
        return int(max(date))

def min_of_date(date : Any) -> int: 
    """Smallest YYYYMMDD int in ``date``; ``99991231`` if empty."""
    if date is None or len(date) == 0:
        return 99991231
    else:
        return int(min(date))