...
def dynamic_members(path : str | Path , 
    ...
    Args:
        path: File or directory passed to ``dynamic_modules``.
        predicate: ``inspect.getmembers`` filter (default: classes and functions).
        subclass_of: If set, keep only non-abstract strict subclasses.
        ignore_imported_members: Skip objects whose ``__module__`` differs from the file module.

    Yields:
        Name and member object pairs.
    """
    for module in dynamic_modules(path):
        for name , obj in inspect.getmembers(module , predicate):
            if ignore_imported_members and module.__name__ !=  getattr(obj , '__module__' , None): 
                continue
            if subclass_of is not None and not true_subclass(obj , subclass_of): 
                continue
            yield name , obj

def true_subclass(cls , base_cls):
    """True if ``cls`` is a concrete (non-abstract) subclass of ``base_cls``."""
