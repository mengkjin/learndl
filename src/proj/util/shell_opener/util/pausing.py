"""Compose shell lines with optional pause-after snippet."""

from __future__ import annotations

import sys

__all__ = ["compose_with_pause"]

_DEFAULT_PAUSE_UNIX = (
    "; echo 'Task complete. Press any key to exit...'; "
    "(read -k 1 2>/dev/null) || read -r -n 1 -s; exit"
)

# cmd.exe: ``timeout /t -1`` waits for a key then returns. With ``cmd /k`` you must ``exit``
# afterward or the pane stays open; ``& exit`` is harmless on a ``cmd /c`` line (already exiting).
_DEFAULT_PAUSE_WIN = (
    "& echo. & echo Task complete. Press any key to exit... & timeout /t -1 >nul & exit"
)

_DEFAULT_CLOSE_UNIX = "; exit"
_DEFAULT_CLOSE_WIN = "& exit"

def compose_with_pause(command: str, *, pause_when_done: bool, close_when_done: bool = False) -> str:
    """Append a platform-appropriate 'press any key to exit' snippet to ``command`` when requested."""
    if pause_when_done and sys.platform == "win32":
        return f"{command} {_DEFAULT_PAUSE_WIN}"
    elif pause_when_done:
        return f"{command} {_DEFAULT_PAUSE_UNIX}"
    elif close_when_done and sys.platform == "win32":
        return f"{command} {_DEFAULT_CLOSE_WIN}"
    elif close_when_done:
        return f"{command} {_DEFAULT_CLOSE_UNIX}"
    else:
        return command
