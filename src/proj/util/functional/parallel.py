...
def parallel(
    ...
    """
    result , func_calls = _FuncCall.from_func_calls(inputs , ignore_error = ignore_error , **kwargs)
    method = _get_method(method if len(func_calls) > 1 else 0 , max_workers)
    ...
    if method == 0:
        ...
        try:
            with ProcessPoolExecutor(
                max_workers=max_workers , mp_context=_process_pool_context() , **mp_kwargs
            ) as pool:
                futures = [(func_call , func_call.submit(pool)) for func_call in func_calls]
                for func_call , future in futures:
...
class _FuncCall:
    ...
    def from_func_calls(cls , inputs : Mapping[Any , INPUT_TYPE[T]] | Iterable[INPUT_TYPE[T]] , 
        ...
        func_calls : list[_FuncCall] = []
        for key , input in iterance:
            func_calls.append(_FuncCall(input , key , result , catch_errors = catch_errors , **kwargs))
