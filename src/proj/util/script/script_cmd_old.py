...
class ScriptCmd:
    ...
    @property
    def real_pid(self):
        return self._get_real_pid(self.process , self._cmd)

    def _create_py_cmd(self):
    ...
    def _create_os_cmd(self):
        self.os_cmd = shlex.split(self.py_cmd)

    def _create_shell_cmd(self):
        if MACHINE.is_linux:
...
'''
    ...
    def run(self):
        if self.mode == 'shell' and MACHINE.is_macos and self.macos_tempfile_method:
            return self._run_in_shell_darwin_tempfile()
        ...
        self._run_cmd(cmd, communicate_input = communicate_input)
        return self

    def _run_in_shell_darwin_tempfile(self):
        ...
        try:
            self._run_cmd([*self.shell_cmd , str(temp_script_path)])
        finally:
            temp_script_path.unlink(missing_ok=True)

        return self

    def _run_cmd(self , cmd : list[str] | str , communicate_input : str | None = None, encoding = 'utf-8'):
        try:
            self.process = self._Popen(cmd, encoding = encoding, communicating = communicate_input is not None)
            if communicate_input is not None:
                self.process.communicate(input = communicate_input)

            if not self.process.returncode:
                Logger.success(f"{self._cmd_type} command executed successfully : {cmd}")
            else:
                Logger.alert2(f"{self._cmd_type} command executed failed : {cmd}")
                Logger.alert2(f"Return Code: {self.process.returncode}")
                Logger.alert2(f"Error Outputs: {self.process.stderr}")
        except subprocess.CalledProcessError as e:
            Logger.error(f"{self._cmd_type} command executed failed with CalledProcessError : {cmd}")
            Logger.error(f"Return Code: {e.returncode}")
            Logger.error(f"Error Outputs: {e.stderr}")

    @staticmethod
    def _Popen(cmd : list[str] | str , encoding = 'utf-8' , communicating = False):
        kwargs = {
    ...
    @classmethod
    def _get_task_id_from_cmd(cls , cmd : str):
        return cmd.split('task_id=')[1].split(' ')[0] if 'task_id=' in cmd else None

    @classmethod
    def _get_real_pid(cls , process : subprocess.Popen | subprocess.CompletedProcess | None , cmd : str):
        task_id = cls._get_task_id_from_cmd(cmd)
        script_name = os.path.basename(cmd.split('.py')[0].split(' ')[-1]) + '.py'
        if MACHINE.is_linux or MACHINE.is_macos:
            return cls._find_python_process_by_name(script_name , task_id = task_id)
        elif MACHINE.is_windows:
            if (python_pid := cls._find_child_python_process_by_name(process, script_name)) is not None:
                return python_pid
            else:
                return cls._find_python_process_by_name(script_name, task_id=task_id)
        else:
    ...
    @classmethod
    def _find_child_python_process_by_name(cls , parent_process: subprocess.Popen | subprocess.CompletedProcess | None, name: str):
    ...
    @classmethod
    def _find_python_process_by_name(cls , name : str , task_id : str  | None = None , try_times : int = 20):
