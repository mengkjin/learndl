...
__all__ = ["compose_with_done_action"]

_WIN_ACTION_SUFFIX : dict[Literal['pause' , 'close' , 'keep'] , str] = {
    'pause': " & echo. & echo Task complete. Press any key to exit... & timeout /t -1 >nul & exit",
    'close': " & exit" ,
    'keep': '',
}
_UNIX_ACTION_SUFFIX : dict[Literal['pause' , 'close' , 'keep'] , str] = {
    'pause': " ; echo 'Task complete. Press any key to exit...'; (read -k 1 2>/dev/null) || read -r -n 1 -s; exit" ,
    'close': " ; exit",
    'keep': '',
}

def compose_with_done_action(command: str, *, done_action: Literal['pause' , 'close' , 'keep'] = 'pause') -> str:
