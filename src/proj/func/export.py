...
def dfs_to_excel(dfs : dict[str , pd.DataFrame] , path : str | Path , mode : Literal['a','w'] = 'w' , 
                 name_prefix = '' , print_prefix = None , indent : int = 1 , vb_level : int | Literal['max','min','inf'] = 3):
...
def figs_to_pdf(figs : dict[str , Figure] , path : str | Path , print_prefix = None , indent : int = 1 , vb_level : int | Literal['max','min','inf'] = 3):
    os.makedirs(os.path.dirname(path) , exist_ok=True)
