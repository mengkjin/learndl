...
class Pipeline(BaseCallBack):
    ...
    @property
    def config(self) -> TrainConfig:    return self.model_module.config
    @property
    def logger(self) -> logging.Logger: return self.model_module.logger
    @property
    ...
    def new_model(self):
        self.tic('model')
        self.model_num = self.model_module.model_num 
        self.model_date = self.model_module.model_date
        self.model_stage += 1
        self.attempt     = -1
        self.epoch_model = -1
    ...
    def loop_continue(self): 
        if self.loop_status in ['attempt' , 'epoch']:
            return True
    ...
    @property
    def loop_new_attempt(self): return self.loop_status == 'attempt'
    ...
    def check_nanloss(self , is_nanloss):
        ...
        if self.stage == 'fit' and is_nanloss: 
            ...
            if self.nanlife > 0:
                self.logger.error(f'Initialize a new model to retrain! Lives remaining {self.nanlife}')
                self.nanlife -= 1
                self.nanloss  = True
                self.loop_status = 'attempt'
                self.attempt -= 1
    ...
    def new_metric(self , dataset , model_type = 'best'):
        ...
        self.metric_batchs.new(dataset , self.model_num , self.model_date , self.epoch , model_type)
    ...
    def update_test_score(self):
        model_type = self.model_module.model_type
        test_dates = self.model_module.data_mod.model_test_dates
        self.test_record.update_score(self.model_num , model_type , self.scores[-len(test_dates):])
    ...
    def toc_str(self , key : str , avg = False): 
        toc = self.toc(key)
        if avg and self.model_stage * self.epoch_stage:
    ...
    @property
    def terminate_cond(self) -> dict: return self.config.train_param['terminate']
    @property
    ...
    def on_train_epoch_start(self):
        self.new_epoch()
        self.new_metric(self.model_module.dataset , 'best')
    def on_train_epoch_end(self):
        opt : Optimizer = self.model_module.optimizer
        ...
        if opt.scheduler_step(self.epoch) == 'reset_learn_rate':
            sdout = f'Reset learn rate and scheduler at the end of epoch {self.epoch} ,' + \
    ...
    def on_test_start(self): 
        ...
        self.test_record = TestRecord(self.config.model_num_list , self.config.model_types , self.logger.info)
    ...
    def on_test_model_start(self):
        ...
        self.model_date = self.model_module.model_date 
        test_dates = self.model_module.data_mod.model_test_dates
        if self.model_num == 0: self.test_record.add_rows(self.model_date , test_dates)
    ...
    def on_test_model_type_start(self):
        self.new_metric(self.model_module.dataset , self.model_module.model_type)
