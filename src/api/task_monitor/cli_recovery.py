...
def register(root: Path, path: Path | None = None) -> bool:
    ...
    with lock(path):
        ...
        if live(state.get('process', {})) and state['process'] != current:
            return False
        state.update({'version': 1, 'uid': os.getuid(), 'root': str(root.resolve()),
                      ...
                      'next_attempt': 0, 'last_error': None, 'failure_paused': False})
        save(path, state)
    return True
...
def session_active(env: dict) -> bool:
    ...
    try:
        ...
        return result.returncode == 0 and fields.get('User') == str(os.getuid()) and fields.get('State') in {
            'active', 'online'
...
def dispatch(state: dict, path: Path) -> None:
    ...
    argv = ['systemd-run', '--user', '--quiet', '--collect',
            f'--unit=learndl-cli-{name}-{uuid.uuid4().hex[:8]}',
...
def tick(path: Path | None = None, *, now: float | None = None) -> dict:
    ...
    with lock(path):
        ...
        if state.get('pending_until'):
            ...
            state['last_error'] = 'CLI did not register within 60 seconds'
...
def launch(path: Path) -> None:
    state = load(path)
    if not state.get('desired') or state.get('paused') or live(state.get('process', {})):
        return
