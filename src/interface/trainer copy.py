...
class ModelTrainer(BaseModelModule):
    ...
    def on_test_batch(self):
        ...
        # if self.status.dataset == 'test' and self.config.lgbt_ensembler:
        self.batch_metrics()
    ...
    def load_model(self , training : bool , model_type = 'best' , lr_multiplier = 1.):
        ...
        if training:
            self.ensembles = EnsembleModels.get_models(self.config.model_types , self.checkpoint , device=self.device)
    ...
    def save_model(self):
        ...
        for model_type in self.model_types:
            model_dict = {'state_dict' : self.ensembles.state_dict(model_type , self.net , self.data_mod.train_dataloader())}
