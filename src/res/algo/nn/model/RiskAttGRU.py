...
class risk_att_gru(nn.Module):
    ...
    def __init__(
            self, 
            input_dim    = (6,len(CONF.RISK['style']),len(CONF.RISK['indus'])),
            hidden_dim   = 2**5,
            att_dim      = 2**7 ,
            dropout      = 0.1,
            act_type     = 'leaky',
        ...
        assert input_dim[1] == len(CONF.RISK['style']) , \
            (input_dim , (len(CONF.RISK['style']),len(CONF.RISK['indus'])))
        assert input_dim[2] == len(CONF.RISK['indus']) , \
            (input_dim , (len(CONF.RISK['style']),len(CONF.RISK['indus'])))
