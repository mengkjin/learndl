...
class SpecificCB_Global2Top(BaseCallBack):
    ...
    def override_configuration(self):
        if (max_epoch := self.config.max_epoch) <= self.protect:
            ...
            self.logger.alert1(f'overrides [max_epoch] {max_epoch} -> {max_epoch + self.protect}' , ind = 1 , vb = 1)
        ...
        if tuple(loss_criteria.keys()) != ('global2top',):
            ...
            self.logger.alert1(f'overrides [loss criteria] {loss_criteria} -> {self.config.criterion_loss}' , ind = 1 , vb = 1)
        ...
        if tuple(accuracy_criteria.keys()) != ('global2top',):
            ...
            self.logger.alert1(f'overrides [accuracy criteria] {accuracy_criteria} -> {self.config.criterion_accuracy}' , ind = 1 , vb = 1)
