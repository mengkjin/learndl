...
def gp_selection(toolbox,evolve_result,gp_inputs,param,records,tensors,i_iter=0,
    ...
    if True: 
        Logger.stdout(f'Cuda Memories of "gp_inputs" take {MemoryManager.object_memory(gp_inputs):.4f}G' , indent = 1)
        Logger.stdout(f'Cuda Memories of "elites"    take {MemoryManager.object_memory(elites):.4f}G' , indent = 1)
        Logger.stdout(f'Cuda Memories of "tensors"   take {MemoryManager.object_memory(tensors):.4f}G' , indent = 1)
