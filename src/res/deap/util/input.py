...
def gp_labels_raw(CP = None , neutral_factor = None , neutral_group = None , nday = 10 , delay = 1 , 
    ...
    labels = T.ts_delay(T.pctchg(CP, nday) , -nday-delay , no_alert = True)  # t+1至t+11的收益率
    neutral_x = T.neutralize_xdata_2d(neutral_factor, neutral_group)
    labels_raw = T.neutralize_2d(labels, neutral_x , inplace=True)  # 市值行业中性化
...
def df2ts(x : pd.DataFrame | torch.Tensor , gp_key = '' , device : torch.device | None = None , share_memory = True) -> torch.Tensor:
    ...
    if isinstance(x , pd.DataFrame): 
        x = torch.FloatTensor(x.values).T
...
class gpInput:
    ...
    def __init__(self , param : gpParameters | None = None , status : gpStatus | None = None , logger : gpLogger | None = None , **kwargs):
    ...
    def load_data(self):
        ...
        package_require = ['argnames' , 'inputs' , 'size' , 'indus' , 'labels_raw' , 'df_index' , 'df_columns' , 'universe']

        load_finished = False
        package_data = torch_load(package_path , map_location = self.device) if package_path.exists() else {}

        if not np.isin(package_require , list(package_data.keys())).all() or not np.isin(self.argnames , package_data['argnames']).all():
            Logger.alert1(f'Exists "{package_path}" but Lack Required Data!' , indent = 1)
            Logger.alert1(f'Required: {package_require}' , indent = 1)
            Logger.alert1(f'Available: {list(package_data.keys())}' , indent = 1)
        else:
            assert np.isin(package_require , list(package_data.keys())).all() , np.setdiff1d(package_require , list(package_data.keys()))
            assert np.isin(self.argnames , package_data['argnames']).all() , np.setdiff1d(self.argnames , package_data['argnames'])
        ...
        if not load_finished:
            ...
            if 'CP' in self.argnames:
                CP = self.inputs[self.argnames.index('CP')]      
            else:
            ...
            Logger.success(f'Package data saved to "{package_path}"' , indent = 1)
    ...
    def update_residual(self , **kwargs):
        ...
        if isinstance(elites , torch.Tensor) and self.param.labels_neut_type == 'svd': 
            ...
            Logger.stdout(f'Elites({elites.shape[-1]}) Shrink to SvdElites({neutra.shape[-1]})' , indent = 1)
