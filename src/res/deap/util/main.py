...
from .syntax import SyntaxRecord , SyntaxControl , BaseIndividual , BaseSyntax
from .toolbox import BaseToolbox
from .params import gpParameters
from .logger import gpLogger
from .elite import gpEliteGroup
...
class gpMain:
    ...
    def update_toolbox(self , **kwargs):
        ...
        ind_pset , syx_pset = Primative.GetPrimSets(self.n_args , self.gp_argnames , '_' if self.i_iter < 0 else f'_{self.i_iter}')
        ...
        creator.create('Individual', BaseIndividual, fitness=getattr(creator , 'FitnessMin'), 
                       pset=ind_pset , __hash__ = lambda self:hash(id(self))) 
        creator.create('Syntax'    , BaseSyntax, fitness=getattr(creator , 'FitnessMin'), ind_str = str , 
                       pset=syx_pset , __hash__ = lambda self:hash(id(self)))
        
        toolbox = BaseToolbox(ind_pset , syx_pset)
        toolbox.register('generate_expr', gp.genHalfAndHalf, pset=ind_pset, min_=1, max_= self.param.max_depth)
        ...
        toolbox.register('expr_mut', gp.genHalfAndHalf, pset=ind_pset , min_=0, max_= self.param.max_depth)  # genFull
        toolbox.register('mutate', gp.mutUniform, expr = getattr(toolbox , 'expr_mut') , pset=ind_pset) 
    ...
    def syntax2value(self , individual : BaseSyntax | BaseIndividual | str | SyntaxRecord , * , process_stream='inf_winsor_norm',
                     **kwargs):
        """
        根据迭代出的因子表达式,计算因子值
    ...
    def evaluate_syx(self , individual : BaseSyntax , pool_skuname : str , * , const_annual = 24 , min_coverage = 0.5 , **kwargs):
        ...
        factor = self.syntax2value(individual , **kwargs)
    ...
    def population(self , pop_num : int , max_round = 100 , last_gen : Sequence[BaseIndividual | BaseSyntax | str | SyntaxRecord] | None = None , forbidden : list[BaseIndividual | BaseSyntax | str | SyntaxRecord] | None = None , **kwargs):
        """
        初始化种群
        input:
    ...
    def evolution(self , * , forbidden_lambda = None , show_progress=__debug__,stats=None,**kwargs):
        ...
        for i_gen in range(start_gen, self.param.n_gen):
            ...
            population = self.toolbox.indpop2syxpop(population)
            #Logger.stdout([str(ind) for ind in population[:20]])
            if show_progress and i_gen == 0: 
            ...
            offspring = self.toolbox.syxpop2indpop(list(set(offspring)))
    ...
    def selection(self , * , show_progress=__debug__ , **kwargs):
        ...
        hof_elites = gpEliteGroup(start_i_elite = len(elite_log) , device=self.device).assign_logs(hof_log = hof_log , elite_log = elite_log)
        infos   = pd.DataFrame(
            [[self.i_iter,-1,SyntaxControl.trim_syntax(ind),ind.if_valid,False,0.] for ind in halloffame] , 
            columns = pd.Index(['i_iter','i_elite','syntax','valid','elite','max_corr']))
        metrics = pd.DataFrame([getattr(ind,'metrics') for ind in halloffame] , columns = self.param.fitness_wgt.keys()) #.reset_index(drop=True)
        ...
        for i , hof in enumerate(halloffame):
            ...
            factor_value = self.syntax2value(hof,**kwargs)
            self.memory.check('factor')
