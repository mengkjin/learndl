...
class gpParameters:
    ...
    @property
    def gp_fac_list(self) -> list[str]:
        """作为GP输入时,标准化因子的数量"""
        return self.params.get('gp_fac_list' , [])
    @property
    def gp_raw_list(self) -> list[str]:
        """作为GP输入时,原始指标的数量"""
    ...
    def n_args(self) -> tuple[int , int]:
        return (len(self.gp_fac_list) , len(self.gp_raw_list))
    @property
    ...
    @property
    def fitness_wgt(self) -> dict[str , float]:
        """因子评价权重"""
