...
from src.proj import PATH , Proj , CALENDAR , Const , Base
...
class TrackingPortfolioManager(Base.BoundLogger):
    ...
    def update(cls , reset_ports : list[str] | None = None , indent : int = 0 , vb_level : Any = 1):
        ...
        cls.logger.stdout(f'Build Tracking Portfolios at {Base.Dates(date)} start ...' , idt = 1 , vb = 1)
        ...
        if len(updated_ports) == 0: 
            cls.logger.skipping(f'No Tracking Portfolios Updated at {Base.Dates(date)}' , idt = 1)
        else:
            ...
            cls.logger.success(f'{len(updated_ports)} Tracking Portfolios Updated at {Base.Dates(date)}: [{", ".join(new_ports.keys())}]' , idt = 1 , vb = 2)
        ...
        cls.logger.stdout(f'Analyze Tracking Portfolios at {Base.Dates(date)} start ...' , idt = 1 , vb = 1)
        
        for name in TrackingPort.candidate_ports:
            tp = TrackingPort.load(name , vb_level = cls.vb_level + 1 , indent = cls.indent + 1)
            with tp.logger.subprocess(idt = 1):
                tp.analyze(key_fig = '')
        cls.logger.success(f'{len(TrackingPort.candidate_ports)} Tracking Portfolios Analyzed at {Base.Dates(date)}' , idt = 1)
