...
class TopStocksPortfolioCreator(PortCreator):
    ...
    def setup(self , print_info : bool = False , **kwargs):
        ...
        self.conf2 = ScreeningPortfolioCreatorConfig.init_from(print_info = print_info , **kwargs)
        return self
    ...
    def solve(self):
        ...
        if amodel.name == 'swalast':
            print(pool.sort_values('secid').head(10))
        ...
        if amodel.name == 'swalast':
            print(df.sort_values('secid'))
            amodel2 = self.conf2.get_sorting_alpha(self.model_date)
            print(amodel2.sort_values('secid').head(10))
            ...
            screening_pool = screening_pool.query('rankpct >= @self.conf2.screen_ratio')
            ...
            pool.loc[:, 'selected'] = pool['weight'].fillna(0) > 0
            pool.loc[:, 'buffered'] = (
                (pool['rankpct'] >= self.conf2.buffer_zone) + (pool['selected'].cumsum() <= self.conf2.stay_num) *
                (pool['rankpct'] >= self.conf2.no_zone))

            stay = pool.query('selected & buffered')
            stay_secid = stay['secid'].to_numpy() # noqa

            stay_ind_count = stay.groupby('indus')['secid'].count()
            stay_ind_count = stay_ind_count.rename('count')
            
            new_pool = pool.query('secid not in @stay_secid').merge(stay_ind_count , on = 'indus' , how = 'left')
            max_ind_rank = self.conf2.indus_slots - new_pool['count'].fillna(0) # noqa
            entry = new_pool.query('ind_rank < @max_ind_rank').sort_values('alpha' , ascending = False).head(self.conf2.n_best - stay.shape[0])
