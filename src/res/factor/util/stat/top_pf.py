...
def _filter_account(account : pd.DataFrame , lag0 = True , pos_model_date = False):
...
def calc_frontface(account : pd.DataFrame):
    df = _filter_account(account)
...
def calc_perf_curve(account : pd.DataFrame):
    df = _filter_account(account).loc[:,['end','pf','bm','excess']]
...
def calc_perf_excess(account : pd.DataFrame):
    return calc_perf_curve(account)

def calc_perf_drawdown(account : pd.DataFrame):
    df = _filter_account(account).loc[:,['end','pf','overnight']]
    index_names = [str(name) for name in df.index.names]
    df = df.sort_values([*index_names , 'end']).rename(columns={'end':'trade_date'})
...
def calc_perf_excess_drawdown(account : pd.DataFrame):
    df = _filter_account(account).loc[:,['end','excess']]
    index_names = [str(name) for name in df.index.names]
    df = df.sort_values([*index_names , 'end']).rename(columns={'end':'trade_date'})
    df['excess'] = eval_cum_ret(df['excess'] , 'lin' , groupby=index_names)
...
def calc_perf_period(account : pd.DataFrame , period : Literal['year' , 'yearmonth' , 'month'] = 'year'):
    ...
    account = _filter_account(account)
...
def calc_perf_year(account : pd.DataFrame):
    '''Calculate performance stats for each year'''
    return calc_perf_period(account , 'year')

def calc_perf_month(account : pd.DataFrame):
