...
class Plotter:
    ...
    def plot_perf_curve(self , data : pd.DataFrame , show = False):
        ...
        for i , sub_data in enumerate(group_plot):     
            with plot.PlotFactorData(sub_data , show = show and i == 0, 
                                    title = f'{self.title_prefix} Accumulative Return') as (df , fig):
    ...
    def plot_perf_excess(self , data : pd.DataFrame , show = False):
        group_plot = plot.PlotMultipleData(data , group_key = ['factor_name','benchmark'])
        for i , sub_data in enumerate(group_plot):     
            with plot.PlotFactorData(sub_data , show = show and i == 0, title = f'{self.title_prefix} Accumulative Excess Return') as (df , fig):
    ...
    def plot_perf_excess_drawdown(self , data : pd.DataFrame , show = False):
        group_plot = plot.PlotMultipleData(data , group_key = ['factor_name','benchmark'])
        for i , sub_data in enumerate(group_plot):     
            with plot.PlotFactorData(sub_data , show = show and i == 0, title = f'{self.title_prefix} Excess Drawdown') as (df , fig):
    ...
    def plot_exp_style(self , data : pd.DataFrame , show = False):
        group_plot = plot.PlotMultipleData(data , group_key = ['factor_name' , 'benchmark'])
        for i , sub_data in enumerate(group_plot):     
            with plot.PlotFactorData(sub_data , drop = [] ,show = show and i == 0, title = f'{self.title_prefix} Style Exposure') as (df , fig):
    ...
    def plot_exp_indus(self , data : pd.DataFrame , show = False):
        group_plot = plot.PlotMultipleData(data , group_key = ['factor_name' , 'benchmark'])
        for i , sub_data in enumerate(group_plot):     
            with plot.PlotFactorData(sub_data , drop = [] ,show = show and i == 0, title = f'{self.title_prefix} Industry Exposure') as (df , fig):
                df = df.groupby(['strategy'] , observed=True).mean().reset_index().\
    ...
    def plot_attrib_source(self , data : pd.DataFrame , show = False):
        group_plot = plot.PlotMultipleData(data , group_key = ['factor_name' , 'benchmark'])
        for i , sub_data in enumerate(group_plot):     
            with plot.PlotFactorData(sub_data , drop = [] , show = show and i == 0, title = f'{self.title_prefix} Cumulative Attribution') as (df , fig):
    ...
    def plot_attrib_style(self , data : pd.DataFrame , show = False):
        group_plot = plot.PlotMultipleData(data , group_key = ['factor_name' , 'benchmark'])
        for i , sub_data in enumerate(group_plot):     
            with plot.PlotFactorData(sub_data , drop = [] , show = show and i == 0 , title = f'{self.title_prefix} Style Attribution') as (df , fig):
