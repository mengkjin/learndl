...
class PlotDfFigIterator:
    ...
    def iter(self):
        ...
        self.args_set = False
        self.group_plot = PlotMultipleData(self.data , self.group_key , self.num_groups_per_iter)
        ...
        for i , sub_data in enumerate(self.group_plot):
            ...
            with PlotFactorData(sub_data , drop = self.drop , title = title , full_title = full_title , show=self.show and i==0 , **self.kwargs) as (df , fig):
                yield df , fig
