...
class AccountConditioner:
    ...
    def conditioned_pf(self) -> pd.Series:
        if (self.account.overnight == 0).all():
            pf = self.position_end * self.account.pf
        else:
...
class BaseConditioner(ABC):
    ...
    def account(self):
        ...
        if hasattr(self , '_account'):
            ...
            self._account = self.portfolio.account
            return self._account
