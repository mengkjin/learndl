...
class DataFailed():
    ...
    def update(self , new):
        assert isinstance(new , DataFailed) , (type(new))
...
class DataTank():
    ...
    def read_dataFDS(self , path , start = None , end = None , dict_only = True) -> None:
        ...
        date = np.array(list(portal.keys())).astype(int)  #type: ignore
        date = date[(date >= start) & (date <= end)]
        secid, pos_secid, _ = _index_union([portal[str(d)]['secid'][:] for d in date])  #type: ignore
        feat , pos_feat , _ = _index_union([portal[str(d)]['feature'][:] for d in date])  #type: ignore
