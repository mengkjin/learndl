...
class ModelData2():
    ...
    """
    def __init__(self , model_data_type , config = None , debug_type = 0):
        self.storage_loader = versatile_storage('mem')
        ...
        if debug_type == 0:
            def set_precision(data):
                if isinstance(data , dict):
                    ...
                    return data.to(torch.float) if isinstance(data , torch.Tensor)  else torch.Tensor(data).to(torch.float)
            self.x_data = {mdt:set_precision(xx.values) for mdt,xx in self.x_data.items()}
            self.y_data , self.norm_param , (self.index_stock , self.index_date) = set_precision(self.y_data.values) , set_precision(self.norms) , self.index 
        else:
    ...
    def create_dataloader(self , process_name , loader_type , model_date , seqlens):
        ...
        self.nonnan_sample = self.cal_nonnan_sample(x, self.y)
    ...
    def new_train_dataloader(self , model_date , seqlens):
        ...
        seqlens = {mdt:(seqlens[mdt] if mdt in seqlens.keys() else 1) for mdt in ['day']}
        self.seq0 = max(seqlens.values())
        self.seq = {mdt:self.seq0 + seqlens[mdt] if seqlens[mdt] <= 0 else seqlens[mdt] for mdt in ['day']}
        model_date_col = (self.index_date < model_date).sum()    
        d0 , d1 = max(0 , model_date_col - 15 - 480) , max(0 , model_date_col - 15)
        self.day_len  = d1 - d0
    ...
    def _train_nonnan_sample(self , x , y):
        ...
        for mdt in ['day']:
            ...
            if mdt in ['day']: nansamp += (x[mdt][:,self.lstepped + self.seq0 - 1][:,:,-1] == 0).sum(-1)
        self.train_nonnan_sample = (nansamp == 0)
        self.tmp_train_nonnan_sample = self.train_nonnan_sample
    ...
    def _train_tv_split(self):
        ...
        train_samples = int(len(ii_stock_wise) * 0.85)
        random.seed(None)
        if 1:
            random.shuffle(ii_stock_wise)
            ii_train , ii_valid = ii_stock_wise[:train_samples] , ii_stock_wise[train_samples:]
        ...
        self.tmp_ii_train , self.tmp_ii_valid = self.ii_train , self.ii_valid
    
    def tensor_standardize_and_weight(self , x, dim=None):
        if x.isnan().all().item():
            return x , x       
    ...
    def _train_y_data(self , y):
        ...
        self.tmp_i_train , self.tmp_i_valid = self.i_train , self.i_valid
        ...
        y_tv , _ = self.tensor_standardize_and_weight(y_tv , dim = 0)
        y_tv = y_tv.reshape(-1,y_tv.shape[-1])
        self.y_train , self.y_valid = (y_tv[self.ii_train] , y_tv[self.ii_valid])
        self.tmp_y_train , self.tmp_y_valid = self.y_train , self.y_valid
    ...
    def _train_dataloader(self , x):
        ...
        """
        self.storage_loader.del_group('train')
        set_iter = [('train' , self.i_train , self.y_train) , ('valid' , self.i_valid , self.y_valid)]

        batch_dir = {'train': './data/train_batch_path', 
                     'valid': './data/valid_batch_path', 
                     'test': './data/test_batch_path'}
        for set_name , set_i , set_y in set_iter:
            batch_sampler = torch.utils.data.BatchSampler(range(len(set_i)) , 10000 , drop_last = False)
            batch_file_list = []
