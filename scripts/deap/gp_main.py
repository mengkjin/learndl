...
def gp_job_dir(test_code = None , job_id = None , train = True , continuation = False):
...
def gp_parameters(test_code = None , job_id = None , train = True , continuation = False , **kwargs):
    '''
    ------------------------ gp parameters ------------------------
    主要遗传规划参数初始化
    ...
    '''
    job_dir = gp_job_dir(test_code , job_id , train , continuation)
...
# %%
def main(test_code = None , job_id = None , start_iter = 0 , start_gen = 0 , **kwargs):
    """
    ------------------------ gp main process ------------------------
    ...
    """
    with Profiler(doso = bool(test_code)) as pfr:
        time0 = time.time()
        gp_params = gp_parameters(test_code , job_id , continuation = start_iter>0 or start_gen>0 , **kwargs)
        gp_space = gp_namespace(gp_params)
        if start_iter == 0:
            gp_space.labels_res = copy.deepcopy(gp_space.labels_raw)
        else:
