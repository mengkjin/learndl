...
def rankic_2d(x , y , dim = 1 , universe = None , min_coverage = 0.5):
    ...
    ic[coverage < min_coverage] = torch.nan
...
def one_hot(x):
    ...
    x = torch.where(nan_index , m + 1 , x)
    dummy = torch.nn.functional.one_hot(x.to(torch.int64)).to(torch.float)[...,:m+1]
    # dummy = torch.stack([(x == i) * 1. for i in range(m+1)] , -1)
    return dummy

def _neutralize_data(y , factors = None , groups = None , auto_intercept = True):
    if factors is None: factors = []
    if groups  is None: groups  = []
    if not isinstance(factors , (list , tuple)): factors = [factors]
    if not isinstance(groups  , (list , tuple)): groups  = [groups]
    if not factors and not groups: return None , y

    x = []
    for f in factors: x.append(f[...,None])
    for g in groups : x.append(one_hot(g)[...,:-1])
    x = torch.cat(x , dim=-1)
    if auto_intercept: x = torch.nn.functional.pad(x , (1,0) , value = 1.)
    x = torch.where(x.isinf() , torch.nan , x)

    y = torch.where(y.isinf() , torch.nan , y)[...,None]
    return x , y
...
def neutralizer(method = 'torch'):
    ...
    def neutralize(y , x):
        if len(y.shape) == len(x.shape)-1: y = y[...,None]
        return (y - x @ betas_func(x , y)).flatten()
    return neutralize

def neutralize_2d(y , factors = None , group = None, dim = 1 , method = 'torch' , auto_intercept = True , silent= True):  # [tensor (TS*C), tensor (TS*C)]
    assert method in ['sk' , 'np' , 'torch']
    x , y = _neutralize_data(y , factors , group , auto_intercept)
    if x is None: return y
    assert dim in [-1,0,1] , dim
    if dim == 0: x , y = x.permute(1,0,2) , y.permute(1,0,2)
    nans  = y.isnan().any(dim=-1) + x.isnan().any(dim=-1) 
    zeros = (x.nan_to_num() == 0).all(dim=dim)
...
def zscore(x , dim = 0 , index = None):
    x_xmean = x - torch.nanmean(x , dim, keepdim=True)  # [TS, C]
    z = x_xmean / (torch.nansum(x_xmean ** 2, dim).sqrt() + 1e-6)
    if index: z = z.select(dim,index)
    return z
