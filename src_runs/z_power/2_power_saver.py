...
def change_power_saver(exclude_scripts : list[str] = [] , log_path = paths[0].joinpath('logs','suspend','power_check.log')):
    running_scripts = get_running_scripts(exclude_scripts)
    do_change = not running_scripts and platform.system() != 'Windows'
    main_str = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ' : Power set to saver'
...
def main():
    change_power_saver(exclude_scripts = ['2_power_saver.py'])
