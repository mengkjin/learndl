...
def change_power_balanced(exclude_scripts : list[str] = [] , log_path = paths[0].joinpath('logs','suspend','power_check.log')):
    # running_scripts = get_running_scripts(exclude_scripts)
    do_change = platform.system() != 'Windows'
    main_str = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ' : Power set to balanced'
...
def main():
    change_power_balanced(exclude_scripts = ['1_power_balanced.py'])
