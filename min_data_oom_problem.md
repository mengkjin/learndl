...
## 11. 2026-09-18 第二次失败：先查证券轴异常

本次日志明确显示旧块 `(39402,2914,1,152)` 占 65.0 GiB，2022 年合并后的
`(47312,3156,1,152)` 占 84.5 GiB。分配后 RSS 从 64.6 GiB 升至 148.9 GiB，
随后在复制旧块阶段退出。数万证券 ID 与预期股票范围不符，不能通过过滤掩盖原因。
没有 kernel/oomd 证据时仍不将 killed 等同于已确认 OOM。

新增只读排查脚本（在服务器项目根目录运行）：

```bash
uv run python scripts/0_check/audit_min_chars.py --start 20100101 --end 20260918
```

如需终端关闭后继续扫描，可用：

```bash
nohup uv run python scripts/0_check/audit_min_chars.py --start 20100101 --end 20260918 > /tmp/learndl-minchars-audit.log 2>&1 &
```

报告默认在本机 `PATH.runtime/min_chars_audit/<时间>/`，启动时打印绝对路径。
仅逐文件读取 secid/date 两列，不重建张量、不改源文件、不过滤证券。

- `summary.json`：各表原始/映射后累计证券数、每年集合大小、首个异常文件。
- `files.jsonl`：逐文件行数、键类型、空键、重复键、文件日期与内容日期、未知 ID 样例、
  新增证券数、累计集合大小及集合摘要；每文件 flush，扫描中断时已完成记录仍可查看。
- `first_seen.csv`：每个原始/映射后 ID 首次出现的文件及是否在历史基础信息表中。
- `mapping.csv`：项目默认 secid 映射改写及其首次出现位置。

单日正常但累计集合不断增长，应从 first_seen 追查 ID 漂移。原始集合正常、映射后异常，
应查当前机器映射配置。原始文件已异常，继续追查其生成链路。基础信息表也可能过期，
unknown 本身不能证明证券种类错误。7000 行仅为可调诊断阈值（`--max-rows`），
不要求历史所有日期恰好 5000 行。退出码 0 表示未发现检查范围内异常，1 表示发现异常，
2 表示读取/映射错误或缺表，扫描不完整。

本地 20250422 三张表证券数为 5126/5147/5126，原始与映射后数量相同；
服务器数据尚未扫描，不能据本地结果判定服务器根因。
重建入口增加键校验和分配预算检查，遇异常抛出可记录错误，不自动丢弃未知证券。
