...
def show_system_info():
    ...
    if torch.cuda.is_available():
        ...
        gpu_info = "**GPU Memory**" , f"{used:.1f} / {total:.1f} GB"
    elif torch.backends.mps.is_available():
        used = torch.mps.current_allocated_memory() / 1024**3
        if torch.__version__ >= '2.3.0':
            recommend = torch.mps.recommended_max_memory() / 1024**3 # type:ignore
            gpu_info = "**MPS Memory**" , f"{used:.1f} / {recommend:.1f} GB"
        else:
            gpu_info = "**MPS Memory**" , f"{used:.1f} GB"
    else:
        gpu_info = "**GPU Memory**" , "No GPU"
    options = {
        ':material/keyboard_command_key: **OS**': f"{platform.system()}" , 
        ':material/memory: **Memory**': f"{psutil.virtual_memory().percent:.1f}%" , 
        f':material/memory_alt: **{gpu_info[0]}**': f"{gpu_info[1]}" , 
        ':material/select_all: **CPU**': f"{psutil.cpu_percent():.1f}%" , 
        ':material/commit: **Python**': f"{sys.version.split('(')[0]}" , 
        ':material/commit: **Streamlit**': f"{st.__version__}" 
    }
